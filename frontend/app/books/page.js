...
export default function BooksPage() {
  ...
  /* -------------------------------------------------------------------------- */
  /* 🧠 Remember selected year in localStorage */
  /* -------------------------------------------------------------------------- */
  useEffect(() => {
      ...
      try {
        ...
        if (res.ok && (data.results?.length || data.length)) {
          ...
          if (Array.isArray(list) && list.length === 1 && list[0]?.isbn) {
            ...
            // 🚀 Guaranteed redirect at browser level
  ...
  return (
    <main className="min-h-screen bg-gray-50 flex flex-col items-center py-10 px-6">
      ...
      {/* 📘 Display grid when multiple books are found */}
            ...
            >
              ...
              <div className="p-5">
                <h3 className="font-semibold text-lg text-blue-900 mb-2 leading-snug">
                  {book.title?.replace(/IGCSET/gi, "IGCSE") ||
                    "Untitled Book"}
                </h3>
                ...
                {/* ✅ Dual Button: View Details + Add to Cart */}
